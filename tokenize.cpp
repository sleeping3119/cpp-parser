...
vector<Token> tokenize(const string &src)
{
    ...
    while (pos < src.size())
    {
        ...
        if (isalpha(c) || c == '_' || (unsigned char)c >= 128)
        {
            ...
            if (keywords.count(acc))
            {
                if (acc == "true" || acc == "false")
                    tokens.push_back({T_BOOLLIT, acc, line, startCol});
                else if (acc == "fn")
                    tokens.push_back({T_FUNCTION, acc, line, startCol});
                else if (acc == "int")
                    tokens.push_back({T_INT, acc, line, startCol});
                else if (acc == "float")
                    tokens.push_back({T_FLOAT, acc, line, startCol});
                else if (acc == "string")
                    tokens.push_back({T_STRING, acc, line, startCol});
                else if (acc == "bool")
                    tokens.push_back({T_BOOL, acc, line, startCol});
                else if (acc == "return")
                    tokens.push_back({T_RETURN, acc, line, startCol});
                else if (acc == "if")
                    tokens.push_back({T_IF, acc, line, startCol});
                else if (acc == "else")
                    tokens.push_back({T_ELSE, acc, line, startCol});
                else if (acc == "for")
                    tokens.push_back({T_FOR, acc, line, startCol});
                else if (acc == "while")
                    tokens.push_back({T_WHILE, acc, line, startCol});
                else if (acc == "break")
                    tokens.push_back({T_BREAK, acc, line, startCol});
                else if (acc == "continue")
                    tokens.push_back({T_CONTINUE, acc, line, startCol});
            }
            else
            {
        ...
        pos++;
        col++;
        if (c == '=' && pos < src.size() && src[pos] == '=')
        {
            val = "==";
            pos++;
            col++;
            tokens.push_back({T_EQUALSOP, val, line, startCol});
            continue;
        }
        else if (c == '+' && pos < src.size() && src[pos] == '+')
        {
            val = "++";
            pos++;
            col++;
            tokens.push_back({T_INCREMENT, val, line, startCol});
            continue;
        }
        else if (c == '+' && pos < src.size() && src[pos] == '=')
        {
            ...
            pos++;
            col++;
            tokens.push_back({T_PLUS_ASSIGN, val, line, startCol});
            continue;
        }
...
string tokenTypeToString(TokenType type)
{
    ...
    case T_EOF:
        return "T_EOF";
    case T_INVALID_IDENTIFIER:
        return "T_INVALID_IDENTIFIER";
    case T_INCREMENT:
        return "T_INCREMENT";
    case T_PLUS_ASSIGN:
